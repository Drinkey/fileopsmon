...
ALL_FLAGS = \
    flags.ACCESS | flags.MODIFY | flags.ATTRIB | flags.CLOSE_WRITE | \
...
READONLY_FLAGS = flags.ACCESS | flags.OPEN | flags.CLOSE_NOWRITE

WRITEONLY_FLAGS = flags.ATTRIB | flags.CLOSE_WRITE |\
    flags.MOVED_FROM | flags.MOVED_TO | flags.CREATE | flags.DELETE| \
        flags.DELETE_SELF | flags.MOVE_SELF | flags.MODIFY

FLAGS = {
    "read": READONLY_FLAGS,
    "write": WRITEONLY_FLAGS,
    "all": ALL_FLAGS
}
...
def get_sub_dirs(root, recursive=False):
    if not recursive:
        return [root]
    dirs = []
    for _dir in os.listdir(root):
        new_path = "{}/{}".format(root, _dir)
        if os.path.isfile(new_path):
            continue
        if os.path.isdir(new_path):
            print("found dir={}".format(new_path))
            dirs.append(new_path)
            dirs.extend(get_sub_dirs(new_path, recursive=recursive))
    return dirs


def watching(watch_dir, inotify, watch_flags, lock):
    print("Adding {} to watchlist".format(watch_dir))
    inotify.add_watch(watch_dir, watch_flags)
    global running
    while running:
        for event in inotify.read(500, 500):
            flagstring = ""
            ...
            try:
                print(
                    "[{timestring}] File={dir}/{name}, FLAGS={flagstring}".format(
                timestring=datetime.datetime.now(), dir=watch_dir, name=event.name, 
...
def main(argv):
    def sig_term_handler(signum, frame):
        print("Caught signal {}, terminating in 2s".format(signum))
        global running
        running = False
        sys.exit(2)
    ...
    watch_targets = args.watch_targets
    recursive = args.recursive
    if args.watchlist_file:
        print("[WARNING!] reading watchlist file, will overwrite directories specified in arguments")
        with open(args.watchlist_file) as fp:
            watch_targets = fp.readlines()
        watch_targets = [f.strip() for f in watch_targets]

    print("setting watch flags to be {} flags".format(args.watch_flags))
    watch_flags = FLAGS[args.watch_flags]

    watch_list = []

    for watch_target in watch_targets:
        for watch_dir in get_sub_dirs(watch_target, recursive=recursive):
            watch_list.append(watch_dir)

    print(watch_list)
    num_of_dirs = len(watch_list)
    print("Total {} dir and sub dirs added to watchlist".format(num_of_dirs))

    print("Starting {} threads polling for events...".format(num_of_dirs))
    threads = []
    mutex = Lock()
    for watch_dir in watch_list:
        inotify = INotify()
        try:
            thread = Thread(target=watching, args=(watch_dir, inotify, watch_flags, mutex))
            thread.start()
        except Exception as exc:
            print("Failed to create new thread, {} is not monitored".format(watch_dir))
            print(exc)
            continue
...
def parse_args(argv):
    parser = argparse.ArgumentParser(description='Monitor file operations in specified dirs.')
    parser.add_argument(
        'watch_targets',
        metavar='N',
        type=str,
    ...
    # BUG: if recursive, the watching targets could be too many, and 
    # will cause threads unable to start
    parser.add_argument(
        '--recursive', "-r",
        dest='recursive',
        action='store_true',
        default=False,
        help='[Danger!]Whether recursively look for sub directories')
    parser.add_argument(
        '--watchlist', '-i',
        dest="watchlist_file",
        default="",
        help="Where read the list of dir to watch"
    )
    parser.add_argument(
        '--flags', '-f',
        ...
        dest="watch_flags",
        default="all",
        help="What file operations wants to watch"
    )
